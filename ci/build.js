...
function pack() {
  ...
  var output = fs.createWriteStream(`${buildPath}/google-calendar-multi-timezone.zip`);
  var archive = archiver('zip', { zlib: { level: 9 } });
  output.on('close', function() {
    console.log(`Artifact: build/google-calendar-multi-timezone.zip (${archive.pointer()} bytes)`);
  });
